...
const TeacherCommunication: React.FC<Props> = ({ user, activeTab }) => {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
       ...
       <Modal isOpen={isNoticeModalOpen} onClose={() => setIsNoticeModalOpen(false)} title="Post Announcement">
         <div className="space-y-4">
           <input placeholder="Title" className="w-full border p-2 rounded" onChange={e => setNewNotice({...newNotice, title: e.target.value})} />
           <textarea placeholder="Content" rows={4} className="w-full border p-2 rounded" onChange={e => setNewNotice({...newNotice, content: e.target.value})} />
